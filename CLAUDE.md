...
Audible reboot signature: every boot runs `STARTUP_COMMANDS "M300 P120 S1200..."` (`Configuration_adv.h:3794`) — a short high 1200 Hz chirp (distinct from the long low 400 Hz homing beep in the OpenPnP `HOME_COMMAND`). A kill-triggered `HAL_reboot()` therefore chirps ~5 s after the kill; watchdog resets chirp immediately. If the operator has never heard an unexplained chirp around incident time, that is evidence *against* the auto-reboot theory and shifts suspicion to electrical step loss (TMC dropout, step-line integrity).
